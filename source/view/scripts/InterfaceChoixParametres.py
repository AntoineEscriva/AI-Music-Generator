...
class Menu(tkinter.Frame):
	...
	def __init__(self, master):
		...
		self.usr_input = tkinter.Entry(self, state='readonly', text=self.affichageChemin, width =30)
		self.usr_input.grid(row=1,column=1, sticky="W")
		...
		#Un espace 
		tkinter.Label(self,text=" ", bg="white").grid(row=9)
		
		#Création et placement du Bouton valider
		self.textBoutonValider = tkinter.StringVar()
		self.textBoutonValider.set("Valider")
		self.Valider = tkinter.Button(self, textvariable=self.textBoutonValider, bg="white", width=20, font = self.texte, bd=1, command=lambda :[self.charging(master)])
		...
		self.textBoutonLecteur.set("Accès au lecteur")
	...
	def changeInterface(self):
		if(self.varButton == 0):
			...
			self.lgSeq.grid(row = 13, column =1, sticky="W")
			self.label12.grid(row=13, column=1, sticky="e")
			self.labelBatch.grid(row = 14, column =0, sticky="W")
			self.boolBatch.grid(row = 14, column =1, sticky="W")
			self.labelSeqBatch.grid(row = 15, column =0, sticky="W")
			self.nbSeqBatch.grid(row = 15, column =1, sticky="W")
			self.label2029.grid(row=15, column=1, sticky="e")
			...
			self.master.geometry("525x550")
			self.labelTaux.grid_forget()
			self.txApprentissage.grid_forget()
			self.label01.grid_forget()
			self.labelEpoch.grid_forget()
			self.nbEpoch.grid_forget()
			self.label17.grid_forget()
			self.labelDimension.grid_forget()
			self.nbDimCachee.grid_forget()
			self.label2421.grid_forget()
			self.labelLayers.grid_forget()
			self.nbLayer.grid_forget()
			self.label15.grid_forget()
			self.labelLongSeq.grid_forget()
			self.lgSeq.grid_forget()
			self.label12.grid_forget()
			self.labelBatch.grid_forget()
			self.boolBatch.grid_forget()
			self.labelSeqBatch.grid_forget()
			self.nbSeqBatch.grid_forget()
			self.label2029.grid_forget()
...
def centrefenetre(fen):
	...
	fen.geometry("%dx%d%+d%+d" % (l,h,(fen.winfo_screenwidth()-l)//2,(fen.winfo_screenheight()-h-100)//2))
