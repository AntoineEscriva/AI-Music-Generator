...
class Menu(tkinter.Frame):
	...
	def __init__(self, master):
		...
		self.configure(bg='white')
		#titre
		self.titre = tkinter.Label(self, text="Configuration", bg='white', font ="Helvetica,30").grid(row=0,column=0)

		#Label sélectionner un dossier
		tkinter.Label(self, text="Sélectionner un dossier",height = hauteurBout, bg="white", font =self.UneFont).grid(row = 1, column = 0, sticky="W")
		...
		self.UF = tkinter.Button(self, text="Choisir", bg = "white", bd=1, width = largeurBout-5,command = lambda:[master.switch_frame(self.Browser())])
		self.UF.grid(row = 1, column = 2, sticky="E")
		
		#Nombre de morceaux
		tkinter.Label(self, text="Nombre de morceaux",height = hauteurBout, bg='white', font =self.UneFont).grid(row = 2, column =0, sticky="W")
		self.nbMorceaux = tkinter.Spinbox(self, from_=1, to=20).grid(row = 2, column =1, sticky="EW")

		# Durée de morceaux
		tkinter.Label(self, text="Durée de morceaux",height = hauteurBout, bg='white', font =self.UneFont).grid(row = 3, column =0, sticky="W")
		
		#Choix de génération
		tkinter.Label(self, text="Choix de génération", width = 15, height = hauteurBout, bg='white', font =self.UneFont).grid(row = 4, column =0, sticky="W")
		...
		#Bouton valider
		self.Valider = tkinter.Button(self, text="Valider", bg="white",font =self.UneFont, bd=1, command=lambda :[master.switch_frame(lecteur.Lecteur)])
